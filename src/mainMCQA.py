...
for model, model_path in MODEL_PATH.items():
    ...
    if "ollama_raw" in model_path:
        model_name = model_path.split("ollama_raw:")[1]
            ...
            for index_sampling in range(MAX_SAMPLING_RATE):
                ...
                if "ollama_raw" in model_path:
                    ...
                    try:
                        ...
                        pass
                    if (model_name == "microsoft/Phi-3-vision-128k-instruct" or model_name == "openbmb/MiniCPM-Llama3-V-2_5")and len(messages) > 1:
                        #drop last message, due to inproper handling of phi 3 model
                        messages.pop()
